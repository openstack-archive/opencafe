...
class _Brew(object):
    def __init__(self, name, fixture, dsl=None, test_classes=None):
        ...
        if test_classes:
            if (not isinstance(test_classes, collections.Iterable) or
                    isinstance(test_classes, string_types)):
                raise BrewMissingTestClassesError(
                    "Brew was instantiated with an uniterable test_classes "
                    "object of type {t}".format(t=type(test_classes)))
            self.test_classes = [
                _ImportablePathWrapper(tc) for tc in test_classes]
