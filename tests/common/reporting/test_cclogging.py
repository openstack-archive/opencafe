...
mock_dict = dict(
    ...
    CAFE_MASTER_LOG_FILE_NAME='tmp')


def mock_os_getenv_verbose(key):
    global mock_dict
    mdict = dict(mock_dict)
    mdict.update(dict(CAFE_LOGGING_VERBOSITY='VERBOSE'))
    return mdict.get(key)


def mock_os_getenv_standard(key):
    global mock_dict
    mdict = dict(mock_dict)
    return mdict.get(key)


def mock_os_getenv_unconfigured(key):
    global mock_dict
    mdict = dict(mock_dict)
    mdict.update(dict(CAFE_LOGGING_VERBOSITY=None))
    return mdict.get(key)
...
class Normal_getLogger_Tests(unittest.TestCase, BaseLoggerTestMixin):
