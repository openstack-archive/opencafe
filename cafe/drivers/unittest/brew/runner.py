...
class BrewRunner(UnittestRunner):
    ...
    def __init__(self):
        ...
        self.brewed_modules = list()

        for brew in self.run_file.iterbrews():
            brewed_module = brew.brew()
            self.brewed_modules.append(brewed_module)

        self.suites = SuiteBuilder(
            testrepos=self.brewed_modules,
