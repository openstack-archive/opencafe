...
class _WriteOnceDict(OrderedDict):
    ...
    """
    def __setitem__(self, key, value, dict_detitem=dict.__setitem__):
        if key in self:
...
class BrewFile(object):
    ...
    def _validate_runfiles(files):
        ...
        for f in files:
            ...
            # Run the configparser with the _WriteOnceDict dict to make sure
            # there are no duplicate items within any given section
            cfg = configparser.SafeConfigParser(dict_type=_WriteOnceDict)
            try:
                cfg.read(f)
            except DuplicateItemException as err:
                msg = "Error in file '{file}': {err}".format(
                    file=f, err=err)
