...
def log_results(result, test_id=None):
    """
    ...
    # Print the tag to test mapping if available
    if hasattr(result, 'mapping'):
        if test_id is not None:
            result.get_tag_mapping().write_to_stream(
                "Test Suite ID: " + test_id + "\n")
        result.get_tag_mapping().print_tag_to_test_mapping()
