...
from cafe.drivers.unittest.result import TaggedTextTestResult
...
class SuiteBuilder(object):
    ...
    def __init__(self, cl_args, roast):
        self.packages = cl_args.packages or []
        self.module_regex = cl_args.module_regex
        self.dotpaths_filters = cl_args.dotpaths_filters
    ...
    def get_classes(self, loaded_module):
        ...
        for objname in dir(loaded_module):
            ...
            if (isclass(obj) and issubclass(obj, unittest.TestCase) and
                ...
                dotpath = "{0}.{1}".format(
                    loaded_module.__name__, obj.__name__)
                add_class = not bool(self.dotpaths_filters)
                for dotpath_filter in self.dotpaths_filters:
                    if search(dotpath_filter, dotpath):
                        add_class = True
                if add_class:
                    classes.append(obj)
        return classes
    ...
    def get_modules(self):
        roast = import_module(self.roast)
        ...
        for importer, modname, ispkg in pkgutil.walk_packages(
            ...
            if not ispkg and modname.startswith(product_path):
    ...
    def get_cl_args(self):
        ...
        *list tests for a product
        ...
        argparser.add_argument(
            ...
            help="Package Filter. Not needed use -D.")

        argparser.add_argument(
            "-m", "--module-regex", "--module",
            default="",
            metavar="<string filter>",
            help="Test module filter. Not needed use -D.")
        ...
        argparser.add_argument(
            ...
            default=[],
            metavar="<string filter>",
            help="Dotpath filter. paths print in () when running tests.")
    ...
    def get_runner(cl_args):
        ...
        if cl_args.parallel:
            test_runner = OpenCafeParallelTextTestRunner(
                stream=sys.stdout, verbosity=int(cl_args.verbose),
