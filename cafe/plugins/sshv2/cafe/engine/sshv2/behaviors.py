...
class SSHBehavior(BaseSSHClass):
    ...
    def create_shell(self):
        self.shell = self.client.create_shell()

    def safe_execute_shell_command(self, cmd, timeout=None):
        """Returns SSHRespnse on error returns None"""
        try:
            if getattr(self, "shell", None) is None:
                self.create_shell()
            r = self.shell.execute_shell_command(cmd, timeout)
            return r
        except:
            self.shell.close()
            del self.shell
