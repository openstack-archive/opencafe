...
def print_exception(file_, method, value, err=None):
    """
        Prints exceptions in a standard format to stderr.
    """
    print("{0}".format("=" * 70), file=stderr)
    print("{0}: {1}: {2}: {3}".format(
        file_, method, value, err), file=stderr)
    print("{0}".format("-" * 70), file=stderr)
    if err is not None:
...
class ConfigAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        ...
        if not os.path.exists(path):
            print_exception(
                "Argument Parser", "ConfigAction",
                "Config does not exist: {0}".format(path))
            exit(1)
...
class DataDirectoryAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        if not os.path.exists(value):
            print_exception(
                "Argument Parser", "DataDirectoryAction",
...
class InputFileAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, value, option_string=None):
        if not os.path.exists(value):
            print_exception(
                "Argument Parser", "InputFileAction",
                "Input File does not exist: {0}".format(value))
            exit(1)
        dic = {}
        count = 0
        ...
        try:
            ...
            print_exception(
                "Argument Parser", "InputFileAction",
                "Line in incorrect format: {0}".format(line))
        if len(lines) != count:
            exit(1)
...
class TagAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if len(values) < 1:
            ...
            print("cafe-runner: error: argument --tags/-t: "
