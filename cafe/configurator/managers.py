...
class EnginePluginManager(object):
    ...
    def install_plugins(cls, plugin_names):
        ...
        for plugin_name in plugin_names:
            print wrapper.fill('... {name}'.format(name=plugin_name))
            plugin_dir = os.path.join(
                EngineDirectoryManager.OPENCAFE_SUB_DIRS.PLUGIN_CACHE,
                plugin_name)
            ...
            try:
                ...
                msg = '* Plugin install failed {0}\n{1}\n'.format(
                    str(cmd), str(exception))

                print wrapper.fill(msg)

            if process and process.returncode != 0:
                print wrapper.fill(standard_out)
                print wrapper.fill(standard_error)
                print '* Failed to install plugin: {0}'.format(plugin_name)
