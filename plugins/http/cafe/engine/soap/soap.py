...
#from cafe.engine.http.client import (BaseClient, cclogging)
...
class BaseSoapClient(BaseClient):
    ...
    def __init__(self, wsdl, endpoint=None, username=None, password=None,
        ...
        cclogging.getLogger('suds').setLevel(cclogging.CRITICAL)
        cclogging.basicConfig(level=cclogging.INFO)
        cclogging.getLogger('suds.client').setLevel(cclogging.DEBUG)
        cclogging.getLogger('suds.transport').setLevel(cclogging.CRITICAL)
        cclogging.getLogger('suds.xsd').setLevel(cclogging.CRITICAL)
        cclogging.getLogger('suds.wsdl').setLevel(cclogging.CRITICAL)
        cclogging.getLogger('suds.resolver').setLevel(cclogging.CRITICAL)
        cclogging.getLogger('suds.metrics').setLevel(cclogging.CRITICAL)
        cclogging.getLogger('suds.').setLevel(cclogging.CRITICAL)
        self.wsdl = wsdl
