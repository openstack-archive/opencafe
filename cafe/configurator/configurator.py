...
class EngineConfigManager(object):
    ...
    def build_engine_config(cls):
        ...
        if os.path.exists(cls.ENGINE_CONFIG_PATH) or os.path.exists(cls._OLD_ENGINE_CONFIG_PATH):
            print cls.wrapper.fill('Updating current config')
            config = cls.update_engine_config()
        else:
            print cls.wrapper.fill('Writing new config')
