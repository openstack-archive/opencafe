...
class EngineConfigManager(object):
    ...
    def write_config_backup(cls, config):
        config_backup_location = "{0}{1}".format(ENGINE_CONFIG_PATH, '.backup')
        print(cls.wrapper.fill(
            "Creating backup of {0} at {1}".format(
                ENGINE_CONFIG_PATH, config_backup_location)))
    ...
    def update_engine_config(cls):
        ...
        config = config or cls.read_config_file(ENGINE_CONFIG_PATH)
    ...
    def build_engine_config(cls):
        ...
        cls.write_and_chown_config(config, ENGINE_CONFIG_PATH)
