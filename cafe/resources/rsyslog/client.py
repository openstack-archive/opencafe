...
class RSyslogClient(object):
    ...
    def build_header(self, app_name, host_name, message_id, priority,
        ...
        head.appname = app_name
        head.hostname = host_name
        head.messageid = message_id
        head.priority = priority
        head.processid = process_id
...
class MessageHandler(object):
    ...
    def cee_dict_to_rsyslog(cls, cee_dict):
        sd = cee_dict.get('native')
        if sd is not None:
            sd = cls.sd_dict_to_syslog_str(sd)

        log = ('<{pri}>{ver} {time} {host} {app} {pid} {msgid} {sd} '
               '{msg}').format(
                   pri=cee_dict.get('pri'),
                   time=cee_dict.get('time') or '-',
                   ver=cee_dict.get('ver'),
                   host=cee_dict.get('host') or '-',
                   app=cee_dict.get('pname') or '-',
                   pid=cee_dict.get('pid') or '-',
                   msgid=cee_dict.get('msgid') or '-',
                   sd=sd or '-',
